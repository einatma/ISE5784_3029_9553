...
public class Turret {
    ...
    Point roofFrontTopRight = new Point(frontBottomLeft.getX() + baseWidth, frontBottomLeft.getY(), frontBottomLeft.getZ() + baseLength);
        ...
        for (int i = 0; i < numOfCrenellationsEachSide; i++) {
            ...
            Box crenellationFront = new Box(roofFrontBottomLeft.add(directionWidth.scale(i*crenellationWidth*2)) , crenellationWidth, crenellationHeight, crenellationLength, directionWidth, directionLength);
            turretGeometries.addAll(crenellationFront.getCubeWigs());
            //add back crenellations
            Box crenellationBack = new Box(roofFrontBottomLeft.add(directionWidth.scale(i*crenellationWidth*2)) , crenellationWidth, crenellationHeight, crenellationLength, directionWidth, directionLength);
            turretGeometries.addAll(crenellationBack.getCubeWigs());
            //add left crenellations
            Box crenellationLeft = new Box(roofFrontBottomLeft.add(directionWidth.scale(i*crenellationWidth*2)) , crenellationLength, crenellationHeight, crenellationWidth, directionWidth, directionLength);
            turretGeometries.addAll(crenellationLeft.getCubeWigs());
            //add right crenellations
            Box crenellationRight = new Box(roofFrontBottomLeft.add(directionWidth.scale(i*crenellationWidth*2)) , crenellationLength, crenellationHeight, crenellationWidth, directionWidth, directionLength);
